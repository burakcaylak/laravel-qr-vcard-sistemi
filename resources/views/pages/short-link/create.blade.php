<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('short-link.create') }}
    @endsection
    ...
    <div class="card">
        <div class="card-header">
            <div class="card-title">
                ...
                <h3 class="fw-bold m-0 d-inline">{{ __('common.create_short_link') }}</h3>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('short-link.store') }}" method="POST" id="short_link_form">
                @csrf
                
                <div class="mb-5">
                    <label class="form-label required">{{ __('common.original_url') }}</label>
                    <input type="url" name="original_url" class="form-control @error('original_url') is-invalid @enderror" value="{{ old('original_url') }}" placeholder="https://example.com" required>
                    @error('original_url')
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title') }}" placeholder="{{ __('common.title') }}">
                    @error('title')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-5">
                        ...
                        <input type="text" name="short_code" class="form-control @error('short_code') is-invalid @enderror" value="{{ old('short_code') }}" placeholder="{{ __('common.short_code_auto') }}">
                        <small class="form-text text-muted">{{ __('common.short_code_hint') }}</small>
                        @error('short_code')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-5" id="password_field" style="display: {{ old('password_protected') ? 'block' : 'none' }};">
                ...
                <div class="row">
                    <div class="col-md-6 mb-5">
                        ...
                        <input type="number" name="qr_code_size" class="form-control" value="{{ old('qr_code_size', 300) }}" min="100" max="1000" step="50">
                    ...
                    <div class="col-md-6 mb-5">
                        ...
                        <select name="qr_code_format" class="form-select">
                            <option value="png" {{ old('qr_code_format', 'png') == 'png' ? 'selected' : '' }}>PNG</option>
                            <option value="svg" {{ old('qr_code_format') == 'svg' ? 'selected' : '' }}>SVG</option>
                        </select>
                    </div>
                </div>

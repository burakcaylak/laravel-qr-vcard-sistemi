...
class Theme
{
    ...
    function includeFavicon()
    {
        ...
        if ($settings->favicon) {
            return sprintf('<link rel="shortcut icon" href="%s" />', asset('storage/' . $settings->favicon));
        }
    ...
    {
        // Eğer type parametresi verilmişse onu kullan, yoksa config'den al
        $iconType = !empty($type) ? $type : config('settings.KT_THEME_ICONS', 'duotone');
        ...
        if ($iconType === 'duotone') {
            ...
            $output = '<'.$tag.' class="ki-'.$iconType.' ki-'.$name.(!empty($class) ? " ".$class : '').'">';
            ...
            $output = '<'.$tag.' class="ki-'.$iconType.' ki-'.$name.(!empty($class) ? " ".$class : '').'"></'.$tag.'>';
        }

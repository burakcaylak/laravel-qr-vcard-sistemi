...
if (!function_exists('storageUrl')) {
    /**
     * Get storage URL for public files
     *
     * @param string $path
     * @return string
     */
...
if (!function_exists('getLogo')) {
    /**
     * Get logo URL for light or dark mode
     *
     * @param string $mode 'light' or 'dark'
     * @return string
     */
    function getLogo($mode = 'light')
    {
        $settings = \App\Models\Settings::getSettings();
        
        if ($mode === 'dark' && $settings->logo_dark) {
            return storageUrl($settings->logo_dark);
        }
        
        if ($mode === 'light' && $settings->logo_light) {
            return storageUrl($settings->logo_light);
        }
        
        // Fallback to default
        return image('logos/default-' . ($mode === 'dark' ? 'dark' : 'light') . '.svg');
    }
}

if (!function_exists('getLoginImage')) {
    /**
     * Get login screen image URL
     *
     * @return string
     */
    function getLoginImage()
    {
        ...
        if ($settings->login_image) {
            return storageUrl($settings->login_image);
        }
        
        // Fallback to default
        return image('misc/auth-bg.png');
    }
}
